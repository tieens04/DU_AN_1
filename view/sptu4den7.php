<div class="contain-products">
    <div class="choosedFilter flexContain">
        ...
        <a href="index.php?act=sanpham">
            <h3>
                Từ 4 - 7 triệu <i class="fa fa-close"></i>
            </h3>
        </a>
    </div>
    <div class="contain-products">
    <ul id="products" class="homeproduct group flexContain">
        ...
        $allsp = loadall_sanpham_tu_4_7($current_page, $item_per_page);
        if (!empty($allsp)) {
...
<div class="pagination">
    <?php if ($current_page > 1): ?>
        <a href="index.php?act=sanphamtu4den7&per_page=<?= $item_per_page ?>&page=<?= ($current_page - 1) ?>" class="pagination-link"><i class="fa fa-angle-left"></i></a>
    <?php endif; ?>

    <?php for ($num = 1; $num <= $totalPages; $num++): ?>
        <a href="index.php?act=sanphamtu4den7&per_page=<?= $item_per_page ?>&page=<?= $num ?>" class="page-item <?= ($num == $current_page) ? 'active' : '' ?>"><?= $num ?></a>
    <?php endfor; ?>

    <?php if ($current_page < $totalPages): ?>
        <a href="index.php?act=sanphamtu4den7&per_page=<?= $item_per_page ?>&page=<?= ($current_page + 1) ?>" class="pagination-link"><i class="fa fa-angle-right"></i></a>
    <?php endif; ?>
</div>